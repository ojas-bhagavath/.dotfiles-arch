...
```bash
# From workspace root
pnpm run build:extension:dev:rspack    # Single build
pnpm run watch:extension:rspack        # Watch mode

# From packages/vscode-pylance
pnpm run rspack        # Single build
pnpm run rspack-dev    # Watch mode
```
...
```bash
# From workspace root
pnpm run watch:extension:release       # Production watch (uses webpack)

# From packages/vscode-pylance
pnpm run webpack-prod                  # Production watch
```
...
```bash
# From packages/vscode-pylance
pnpm run rspack -- --mode production

# Or from workspace root
pnpm run build:extension:dev:rspack -- --mode production
```
...
**For maximum obfuscation** (release builds): Use `rspack.config.js` in production mode (`pnpm run webpack-prod`)
